...
var WELCOME_MESSAGE = "I <emphasis level='strong'>love</emphasis> to gaze at the stars. Ask me if the weather is good for star gazing in your zipcode. You can add a specific hour or future date, too, if you want.";
...
var HELP_REPROMPT = "Try this: is the weather good for star gazing? and add your zipcode.";
...
var handlers = {
    ...
    'GetZipcodeIntent': function (event) {
        ...
        this.emit(':ask', "Thanks for giving me your zipcode! You can ask, can I star gaze next Monday at 11pm, or is the weather good for star gazing?"
    },
                  ...
                  for (var i = 0; i < myResult.forecast.forecastday.length; i++) {
                      if(myResult.forecast.forecastday[i].date == date){
                        ...
                        sunset = sunset[1] + "pm";
                          ...
                          for (var j = 0; j < byHourDate.length; j++) {
                            console.log("in J loop");
                            if(byHourDate[j].time.includes(time)){
                              console.log("in time match");
                              console.log(byHourDate[j].is_day);
                                if(byHourDate[j].is_day == 0){
                                  console.log("inside if night");
                                    time = time.split(":");
                                    time = time[0];
                                    newTime = makeItAMPM(time);

                                    speechOutput = "At " + newTime + " on " + month + day +" the weather condition is forecasted as " + byHourToday[j].condition.text;
                                    return speechOutput;
                                };
                            } else {
